...
export default function CartItem({ item, onUpdateQuantity, onRemove }) {
  ...
  return (
    <motion.div layout className="group flex gap-4 sm:gap-5 py-5 border-b border-charcoal/10 last:border-b-0">
      <Link to={itemLink} className="shrink-0 overflow-hidden rounded-xl" aria-label={`Edit ${item.name}`}>
        <img src={item.image} alt={item.name} className="w-20 h-20 sm:w-24 sm:h-24 object-cover transition duration-700 group-hover:scale-105" />
      </Link>
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-3">
          <Link to={itemLink} className="transition duration-300 hover:text-olive">
