...
function DesktopNavLink({ link }) {
  return (
    <NavLink key={link.to} to={link.to} end={link.to === '/'} className="group relative px-1 py-2 text-xs xl:text-sm uppercase tracking-label text-charcoal/70 transition duration-300 hover:text-olive">
      {({ isActive }) => (
        <>
      ...
      <Container className="flex items-center justify-between py-4">
        ...
        <div className="flex items-center gap-4 sm:gap-5">
          ...
          <motion.div whileHover={{ scale: 1.07, rotate: -2 }} whileTap={{ scale: 0.95 }}>
            <Link to="/cart" aria-label="View cart" className="relative block text-charcoal/70 transition duration-300 hover:text-olive">
              ...
              {NAV_LINKS.map((link, index) => (
                ...
                >
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      `block rounded-xl px-2 py-3 text-sm uppercase tracking-label border-b border-charcoal/5 transition duration-300 ${
