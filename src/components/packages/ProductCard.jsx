...
export default function ProductCard({ product }) {
  ...
  return (
    ...
    >
      <Link to={`/packages/${product.id}`} className="block overflow-hidden" aria-label={`View ${product.name} details`}>
        <img
          src={cardImage}
          alt={product.name}
      ...
      <div className="p-4 sm:p-5 flex flex-col flex-1">
        ...
        <Button to={`/packages/${product.id}`} variant="outline" size="sm" className="mt-auto" fullWidth>
          View Details
        </Button>
      </div>
    </motion.article>
