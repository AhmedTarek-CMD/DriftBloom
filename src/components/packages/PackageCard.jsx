...
import { fadeUp } from '../common/Motion.jsx'
import { isBestSellerProduct } from '../../data/products.js'
...
export default function PackageCard({ product }) {
  ...
  return (
    ...
    >
      <Link to={`/packages/${product.id}`} className="relative block overflow-hidden" aria-label={`View ${product.name} package`}>
        <img
          src={cardImage}
          alt={product.name}
      ...
      <div className="p-4 sm:p-5 flex flex-col flex-1">
        ...
        <Button to={`/packages/${product.id}`} size="sm" className="mt-auto" fullWidth>
          View Package
        </Button>
      </div>
    </motion.article>
