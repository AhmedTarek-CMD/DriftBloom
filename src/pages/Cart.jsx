...
function validateForm(form) {
  ...
  else if (form.address.trim().length < 8) errors.address = 'Please enter a more complete address.'
...
export default function Cart() {
  ...
  const handleConfirmOrder = () => {
    ...
    window.setTimeout(() => {
      ...
      setReviewOpen(false)
      clearCart()
      showToast(`Order placed successfully. Instruction language: ${instructionLanguage}.`)
      window.scrollTo({ top: 0, behavior: 'smooth' })
    }, 650)
  }
    ...
    return (
      <Container className="py-20 max-w-2xl">
        ...
        >
          ...
          <Button to="/packages" className="mt-8">
            Continue Shopping
          </Button>
        </motion.div>
      </Container>
      ...
      <Container className="py-20 text-center max-w-2xl">
        <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="rounded-3xl border border-gold/15 bg-white/70 p-8 shadow-soft sm:p-12">
          ...
          <div className="flex flex-col justify-center gap-3 sm:flex-row">
            <Button to="/packages">Browse Packages</Button>
            <Button to="/build-your-package" variant="outline">Build Your Package</Button>
          </div>
        </motion.div>
      </Container>
    )
  ...
  return (
    <Container className="py-10 sm:py-14">
      ...
      {!isAuthenticated && (
        <Reveal className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-beige border border-gold/20 rounded-2xl px-5 py-4 mb-8 shadow-soft">
          ...
          <div className="flex items-center gap-3 shrink-0">
            <Button to="/login" state={{ from: '/cart', message: 'Log in for faster checkout and order tracking.' }} variant="outline" size="sm">
              Log In
            </Button>
            <Button to="/register" state={{ from: '/cart', message: 'Create an account for faster checkout.' }} size="sm">
              Create Account
            </Button>
          </div>
        </Reveal>
      ...
      <div className="mt-8">
        <Link to="/packages" className="text-sm text-olive underline decoration-gold/60 underline-offset-4 transition duration-300 hover:text-olive-dark">
          ← Continue Shopping
        </Link>
      </div>
    </Container>
